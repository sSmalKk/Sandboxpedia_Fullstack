...
export const Cubes = ({ cubes, texture }) => {
...
export function Cube({ position, faceIndex, onClick, texture, hasCubeLeft, hasCubeRight, hasCubeTop, hasCubeBottom, hasCubeFront, hasCubeBack }) {
  const ref = useRef();
    ...
    <group position={position} receiveShadow castShadow  onClick={handleMouseClick} ref={ref}>
      {!hasCubeLeft && (
        <mesh key={0} position={[-0.5, 0, 0]} rotation={[0, -Math.PI / 2, 0]} onPointerOut={() => handleMouseLeave(0)} onPointerMove={() => handleMouseEnter(0)}>
          <planeGeometry />
          <meshStandardMaterial attach="material" map={texture || defaultTexture} color={hoveredFace === 0 ? "hotpink" : "white"} />
        </mesh>
      )}
      {!hasCubeRight && (
